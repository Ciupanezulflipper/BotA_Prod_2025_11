...
def fetch_from_twelvedata(symbol, interval, limit=100):
    ...
    df = df.drop(columns=[c for c in df.columns if c not in ["time","open","high","low","close","volume"]])
    # numeric
...
def fetch_from_alphavantage(symbol, interval, limit=100):
    ...
    if not keys:
        raise RuntimeError(f"alphavantage: {data.get('Note') or data.get('Error Message') or 'no series'}")
    ts = data[keys[0]]
...
def get_ohlc(symbol, interval="1h", limit=100, provider=None):
    ...
    for p in order:
        ...
        if not ok:
            log(f"UTC {datetime.utcnow().strftime('%H:%M')} | SKIP {p.upper()} rate {used}/{cap} this minute")
            continue

        try:
            log(f"UTC {datetime.utcnow().strftime('%H:%M')} | Trying {p.upper()} {symbol} interval={interval} size~{limit}")
            if p == "twelvedata":
