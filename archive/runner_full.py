...
def _now_iso_z() -> str:
    # Preserve EXACT prior log format: YYYY-mm-ddTHH:MM:SSZ
...
# === LOG HELPERS ===
def log_info(msg):
    print(f"[INFO] {_now_iso_z()} | {msg}", flush=True)

def log_error(msg):
    print(f"[ERROR] {_now_iso_z()} | {msg}", flush=True)
...
def run_once(last_seen_dt, last_sent_dt):
    ...
    if not is_trading_hours(dt.datetime.now(UTC)):
...
def main():
    log_info(
        f"Runner starting… | Py {sys.version.split()[0]} | "
        f"ADX_MIN={ADX_MIN}, RSI_BUY={RSI_BUY}, RSI_SELL={RSI_SELL}, MACD_MIN={MACD_MIN}, "
        f"CONF_MIN={CONF_MIN}, THROTTLE_SEC={THROTTLE_SEC}, LOOP_SEC={LOOP_SEC}"
    )
